...
export const MainVideoEditEn = () => {
  ...
  const [seriesEn, setseriesEn] = useState(null);
  const [newValues, setNewValues] = useState({});

     
  const handleNumSeriesChangeMainEn = (event) => {
    ...
    uploadTask.on(
      ...
      () => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
          const newSeriesEn = { ...seriesEn };
          newSeriesEn[`videoUrl${index}`] = downloadURL;
          setseriesEn(newSeriesEn);
  ...
  value={newValues[`episode${i}`] || seriesEn?.[`episode${i}`] || ""}
  onChange={(e) =>
    setNewValues({
      ...newValues,
...
/>
  ...
  <div className="video-container" >
    ...
    {seriesEn && seriesEn[`videoUrl${i}`] && (
      <div>
        <video style={{ width: "200px" }} src={seriesEn[`videoUrl${i}`]} controls alt="property" />
      </div>
    )}
...
<div className="container">
  ...
  type="number"
  value={numSeriesMain}
  onChange={handleNumSeriesChangeMainEn}
/>
    </div>
...
export default MainVideoEditEn;