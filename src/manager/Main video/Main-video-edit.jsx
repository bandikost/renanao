...
export const MainVideoEdit = () => {
  ...
  const [seriesRu, setseriesRu] = useState(null);
  const [newValues, setNewValues] = useState({});

  const handleNumSeriesChangeMain = (event) => {
  ...
  useEffect(() => {
    document.title = t('Video Russian Edit');
  }, []);
    ...
    uploadTask.on(
      ...
      () => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
          const newseriesRu = { ...seriesRu };
          newseriesRu[`videoUrl${index}`] = downloadURL;
          setseriesRu(newseriesRu);
  ...
  value={newValues[`episode${i}`] || seriesRu?.[`episode${i}`] || ""}
  onChange={(e) =>
    setNewValues({
      ...newValues,
...
/>
  ...
  <div className="video-container" >
    ...
    {seriesRu && seriesRu[`videoUrl${i}`] && (
      <div>
        <video style={{ width: "200px" }} src={seriesRu[`videoUrl${i}`]} controls alt="property" />
      </div>
    )}
...
<div className="container">
  ...
  type="number"
  value={numSeriesMain}
  onChange={handleNumSeriesChangeMain}
/>
    </div>
