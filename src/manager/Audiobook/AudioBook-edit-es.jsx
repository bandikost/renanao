...
export const AudiobookEditEs = () => {
  ...
  const [audiobookEs, setaudiobookEs] = useState(null);
  const [newValues, setNewValues] = useState({});

     
  const handleNumSeriesChangeAudioEs = (event) => {
  ...
  useEffect(() => {
    document.title = t('Audio Spanish Edit');
  }, []);
    ...
    uploadTask.on(
      ...
      () => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
          const newaudiobookEs = { ...audiobookEs };
          newaudiobookEs[`videoUrl${index}`] = downloadURL;
          setaudiobookEs(newaudiobookEs);
  ...
  value={newValues[`episode${i}`] || audiobookEs?.[`episode${i}`] || ""}
  onChange={(e) =>
    setNewValues({
      ...newValues,
...
/>
  ...
  <div className="video-container" >
    ...
    {audiobookEs && audiobookEs[`videoUrl${i}`] && (
      <div>
        <video style={{ width: "200px" }} src={audiobookEs[`videoUrl${i}`]} controls alt="property" />
      </div>
    )}
...
<div className="container">
  ...
  type="number"
  value={numSeries}
  onChange={handleNumSeriesChangeAudioEs}
/>
    </div>
...
export default AudiobookEditEs;