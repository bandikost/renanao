...
export const AudiobookEditEn = () => {
  ...
  const [audiobookEn, setaudiobookEn] = useState(null);
  const [newValues, setNewValues] = useState({});

  const handleNumSeriesChangeAudioEn = (event) => {
  ...
  useEffect(() => {
    document.title = t('Audio English Edit');
  }, []);
    ...
    uploadTask.on(
      ...
      () => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
          const newaudiobookEn = { ...audiobookEn };
          newaudiobookEn[`videoUrl${index}`] = downloadURL;
          setaudiobookEn(newaudiobookEn);
  ...
  value={newValues[`episode${i}`] || audiobookEn?.[`episode${i}`] || ""}
  onChange={(e) =>
    setNewValues({
      ...newValues,
...
/>
  ...
  <div className="video-container" >
    ...
    {audiobookEn && audiobookEn[`videoUrl${i}`] && (
      <div>
        <video style={{ width: "200px" }} src={audiobookEn[`videoUrl${i}`]} controls alt="property" />
      </div>
    )}
...
<div className="container">
  ...
  type="number"
  value={numSeries}
  onChange={handleNumSeriesChangeAudioEn}
/>
    </div>
...
export default AudiobookEditEn;