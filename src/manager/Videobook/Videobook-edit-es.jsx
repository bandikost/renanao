...
export const VideobookEditEs = () => {
  ...
  const [videobookEs, setvideobookEs] = useState(null);
  const [newValues, setNewValues] = useState({});

  const handleNumSeriesChangeVideoEs = (event) => {
    ...
    uploadTask.on(
      ...
      () => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
          const newvideobookEs = { ...videobookEs };
          newvideobookEs[`videoUrl${index}`] = downloadURL;
          setvideobookEs(newvideobookEs);
  ...
  value={newValues[`episode${i}`] || videobookEs?.[`episode${i}`] || ""}
  onChange={(e) =>
    setNewValues({
      ...newValues,
...
/>
  ...
  <div className="video-container" >
    ...
    {videobookEs && videobookEs[`videoUrl${i}`] && (
      <div>
        <video style={{ width: "200px" }} src={videobookEs[`videoUrl${i}`]} controls alt="property" />
      </div>
    )}
...
<div className="container">
  ...
  type="number"
  value={numSeries}
  onChange={handleNumSeriesChangeVideoEs}
/>
    </div>
...
export default VideobookEditEs;